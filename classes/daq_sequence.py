...
class _ChannelSequence:
    ...
    def get_val_array(self):
        ...
        for i in range(0, num_intervals):
            ...
            # Note we use t_interval[0] and t_interval[-1] rather than t_0 and t_1.  This is because the function fits expected values
            ...
            try:
                ...
                v_segments.append(np.array(list(map(change_func, t_interval)), dtype=np.float64))
            except IndexError:
                # If t_interval is an empty list we'll catch that here - it just means that t_0 and t_1
                # are so close together (or identical) so no times in t_span are between them.  The card
                # can't update that quick so there is nothing to add to v_segments anyway.
                pass

        return np.concatenate(v_segments) if v_segments else np.array([], dtype=np.float64)
