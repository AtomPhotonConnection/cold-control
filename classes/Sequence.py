...
class Sequence:
    ...
    def addChannelSeq(self, chNum, tV_pairs={0.0: 0.0}, V_intervalStyles=[]):
        ...
        for chNum in self.chSeqs:
            try:
                ...
                if chNum in channelsToUpdate:
                    self.updateChannel(
                        chNum, channelsToUpdate[chNum][0], channelsToUpdate[chNum][1]
                    )
                else:
...
class _ChannelSequence:
    ...
    def __init__(
        self,
        parentSequence,
        tV_pairs: list[tuple[float, float]] = [(0.0, 0.0)],
        V_interval_styles: list[int] = [],
    ):
    ...
    def getValArray(self):
        ...
        for i in range(0, numIntervals):
            ...
            try:
                changeFunc = self.getChangeFunc(
                    changeStyle, (t_interval[0], V_0), (t_interval[-1], V_1)
                )
                if changeFunc == None:
                    raise ValueError(f"Invalid change style {changeStyle} provided")
                V_span = np.append(
                    V_span, np.array(list(map(changeFunc, t_interval)), dtype=np.float64)
                )
            except IndexError:
    ...
    def validate(self):
        ...
        if (
            ...
            raise InvalidSequenceChannelException(
