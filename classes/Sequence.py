    ...
    def updateTimeSteps(self, n_samples, t_step, channelsToUpdate={}):
        ...
        if validationErrors != []:
            raise MultipleInvalidSequenceChannelException(
                    f'{len(validationErrors)} validation errors were found when updating the sequence',
...
#         else:
        for i in range(0, numIntervals):
            ...
            try:
                ...
                if changeFunc == None:
                    raise ValueError(f'Invalid change style {changeStyle} provided')
    ...
    def validate(self):
        ...
        if len(self.tV_pairs) == len(self.V_interval_styles) + 1 and self.tV_pairs[-1][0] != self.parent.getTimeSteps()[-1]:
            raise InvalidSequenceChannelException(f"Sequence validation error: If {len(self.tV_pairs)} time-voltage pairs and {len(self.V_interval_styles)} styles of how to move between them are provided - the last time must be the final data point of the sequence ({self.parent.getTimeSteps()[-1]}).")

        elif len(self.tV_pairs) == len(self.V_interval_styles):
            if self.tV_pairs[-1][0] >= self.parent.getTimeSteps()[-1]:
                raise InvalidSequenceChannelException(f"Sequence validation error: If the same number of time-voltage pairs and interval styles ({len(self.tV_pairs)}) are provided - the final time provided must be before the end of the sequence length..")
            elif self.V_interval_styles[-1] != IntervalStyle.FLAT:
                raise InvalidSequenceChannelException(f"Sequence validation error: If the same number of time-voltage pairs and interval styles ({len(self.tV_pairs)}) are provided - the final interval style must be Flat.")

        elif len(self.tV_pairs) != len(self.V_interval_styles) and len(self.tV_pairs) != len(self.V_interval_styles) + 1:
            raise InvalidSequenceChannelException(f"Sequence validation error: If is not possible to create a sequence out from {len(self.tV_pairs)} time-voltage pairs and {len(self.V_interval_styles)} interval styles.")

        if int(self.tV_pairs[0][0]) != 0:
            raise InvalidSequenceChannelException(f"Every channels sequence must begin at t=0 (here t={self.tV_pairs[0][0]} is the first time provided).")

        if self. tV_pairs[-1][0] > self.parent.getLength():
            raise InvalidSequenceChannelException(f"A channel sequence cannot be created with a time-voltage pair ({self. tV_pairs[-1]}) outside the total running time of it's parent sequence ({self.parent.getLength()})")
