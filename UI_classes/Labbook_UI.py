    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dropdownOptions = self.sortDates(
            [y.group() for y in [r.match(x) for x in os.listdir(self.labbook_dir)] if y is not None]
        )
        ...
        self.refreshButton = ImageButton(
            topFrame, image=icon, command=self.configureForCurrentDate, height=20, width=20
        )
