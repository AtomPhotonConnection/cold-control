...
def calibrate_percentage_power(
    daq_controller,
    chNum_to_calibrate,
...
def calibrate_awg_driven_aom_response(
    ...
    for freq in freqs:
        ...
        print(
            "Calibration limits set to ",
            calibration_lims,
            "to avoid a maximum in the middle of the calibration range.",
        )
...
if __name__ == "__main__":
    ...
    for freq, v in zip(aom_freqs, freq_v):
        ...
        time.sleep(3)
        vData, calData, units = calibrate_absolute_power(
            daq_controller,
            amp_channel,
            (0.2, 1.75),
        ...
        create_calibration_file(
            os.getcwd() + f"/calibrations/jan/{aom_name}/{calibName}", vData, calData, units
        )
        save_calibration_plot(
            os.getcwd() + f"/calibrations/jan/{aom_name}/{calibName}_plot.png",
            vData,
            calData,
            units,
            f"freq = {freq}MHz",
