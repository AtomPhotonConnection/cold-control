...
def create_waveform_lists(waveforms, waveform_sequence, awg_chs):
    ...
    queud_markers = []
    seq_waveforms = [[waveforms[i] for i in ch_waveforms] for ch_waveforms in waveform_sequence]
...
def write_markers(marker_data, awg: WX218x_awg, awg_chs, marker_width):
    ...
    marker_starts = [
        x[0]
        for x in enumerate(zip([0] + marker_data[:-1], marker_data))
    ...
    awg.configure_marker(
        awg_chs[1],  # changed to channel 2 to investigate pulses
        index=2,
...
def run_awg(awg_config: AwgConfiguration, photon_config: PhotonProductionConfiguration):
    ...
    for channel, waveform_data, waveforms, delay, channel_abs_offset in zip(
        awg_config.waveform_output_channels, wf_data, wf_list, wf_stitched_delays, abs_offsets
    ):
        """
            ...
            for ind, waveform in enumerate(waveforms):
                ...
                if len([x for x in queud_markers if x >= 0]) > 0:
                    ...
                    marker_data[marker_index : marker_index + len(wrapped_marker_data)] = [
                        MARKER_WF_LEVS[1]
                        if (a == MARKER_WF_LEVS[1] or b == MARKER_WF_LEVS[1])
                        else MARKER_WF_LEVS[0]
                        if (a == MARKER_WF_LEVS[0] and b == MARKER_WF_LEVS[0])
                        else a + b
                        for a, b in zip(
