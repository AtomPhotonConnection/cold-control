...
def write_channels(awg_chs, _rel_offsets, _wf_data, _awg: WX218x_awg):
    ...
    for channel, rel_offset, data in zip(awg_chs, _rel_offsets, _wf_data):
        ...
        if channel in [
            ...
            Channel.CHANNEL_2,
            Channel.CHANNEL_3,
        ]:  # , Channel.CHANNEL_4]: Why isn't CH4 working?
...
def run_awg(awg_config: AwgConfiguration, dev_mode: bool = False):
    ...
    wf_list, wf_data, wf_stitched_delays, seq_marker_data, queud_markers = create_waveform_lists(
        wfs, wf_seq, awg_config.waveform_output_channels
    )
    ...
    if awg_config.interleave_waveforms:
        wf_stitched_delays = stitch_waveforms(
            awg_config.waveform_output_channels, wf_del, wfs, wf_stitched_delays
        )
    else:
            ...
            if len(waveforms) == 1:
                ...
                if delay < 0:
                    waveform_data += [0] * abs(delay) + waveform.get(
                        sample_rate=awg_config.sample_rate, constant_voltage=constant_V
                    )
