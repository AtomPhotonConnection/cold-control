...
if __name__ == "__main__":
    ...
    config = ConfigObj(
        path_to_config
    )  # permite acceder a los valores de configuración como un diccionario.
    ...
    for x, v in config["waveforms"].items():
        ...
        waveforms.append(
            Waveform(
                fname=v["filename"], mod_frequency=float(v["modulation frequency"]), phases=phases
            )
        )  # map(float, v['phases'])))
    ...
    awg_config = AwgConfiguration(
        ...
        waveforms=waveforms,
        waveform_stitch_delays=list(
            eval(config["waveform stitch delays"])
        ),  #  Retrasos entre formas de onda.
        ...
        waveform_output_channel_lags=list(
            map(float, config["AWG"]["waveform output channel lags"])
        ),  # Retrasos asociados a los canales de salida.
