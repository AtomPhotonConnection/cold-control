    ...
    for channel, waveform_data, waveforms, delay, channel_abs_offset in \
        ...
        for ind, waveform in enumerate(waveforms):
            ...
            if not waveform_aom_calibs:
                ...
                print(f'\tFor waveform with freq {waveform.get_mod_frequency()*10**-6}MHz, using calib for {min(waveform_aom_calibs, key=lambda calib_freq: np.abs(calib_freq - waveform.get_mod_frequency()*10**-6))}MHz')
            ...
            print(f'\tWriting waveform {os.path.split(waveform.fname)[1]} with stitch delay {delay}')
            if len(waveforms)==1:
...
#         if marker_data[-1]==1: marker_data[-1]=0
    ...
    for x in seq_waveform_data:
        if N % 16 != 0:
            print(f'{16-(N % 16)} points added to waveform data to ensure a multiple of 16 points are written to the AWG.')
            x += [0]*(16-(N % 16))
