...
def configure_awg(
    awg_config: AwgConfiguration, photon_production_config: PhotonProductionConfiguration
):
    ...
    channel_absolute_offsets = [
        np.rint(x * 10**-6 * awg_config.sample_rate)
        for x in awg_config.waveform_output_channel_lags
    ]
    channel_relative_offsets = list(
        map(lambda x, m=max(channel_absolute_offsets): int(m - x), channel_absolute_offsets)
    )
    ...
    def get_waveform_calib_fnc(calib_fname, max_eff=0.9):
        ...
        calib_data[:, 1] /= 100.0  # convert % as saved to decimal efficiencies
        calib_data = calib_data[
            (calib_data[:, 1] <= max_eff)
        ]  # remove all elements with greater than the maximum efficiency
    ...
    def get_multiwaveform_marker_data(
        inp_data,
        marker_positions=[],
        marker_levels=(0, 1),
        marker_width=50,
        ...
        """
        data = np.array(
            [marker_levels[0]] * (n_pad_left + inp_data + n_pad_right)
        ...
        if (
            ...
            data[0] = (
                0  # then the channel remains high at the end of the sequence. Don't know why...
            )
    ...
    queud_markers = []
    seq_waveforms = [
        [photon_production_config.waveforms[i] for i in ch_waveforms]
    ...
    if photon_production_config.interleave_waveforms:
        ...
        for i in range(len(awg_chs)):
            ...
            if photon_production_config.waveform_stitch_delays[i][0] == -1:
                ...
                raise ValueError(
                    "Invalid stitch delay needs to be before (-1) or after (+1) the waveform"
                )
    ...
    for channel, waveform_data, waveforms, delay, channel_abs_offset in zip(
        ...
        print(
            "For {0} using aom calibrations in {1}".format(
        ...
        for ind, waveform in enumerate(waveforms):
            ...
            print(
                f"\tWriting waveform {os.path.split(waveform.fname)[1]} with stitch delay {delay}"
            )
            if len(waveforms) == 1:
                print("\tOnly one waveform in sequence")
                # write delay to the single waveform in a sequence
                if delay < 0:
                    waveform_data += [0] * abs(delay) + waveform.get(
                        sample_rate=awg_config.sample_rate,
            ...
            print(f"Marker data length: {len(marker_data)}")
        ...
        if queud_markers != []:
            ...
            for ind, waveform in enumerate(waveforms):
                ...
                if len([x for x in queud_markers if x >= 0]) > 0:
                    ...
                    markers_in_waveform = [
                        x for x in queud_markers if marker_index <= x <= marker_index + seg_length
                    ]
                    ...
                    marker_data[marker_index : marker_index + len(wrapped_marker_data)] = [
                        marker_waveform_levs[1]
                        if (a == marker_waveform_levs[1] or b == marker_waveform_levs[1])
                        else marker_waveform_levs[0]
                        if (a == marker_waveform_levs[0] and b == marker_waveform_levs[0])
                        else a + b
                        for a, b in zip(
                            marker_data[: len(wrapped_marker_data)], wrapped_marker_data
                        )
                    ]
        ...
        if channel in awg_config.marked_channels:
            if seq_marker_data == []:
                ...
                if j1 < j2:
                    seq_marker_data = [
                        sum(x) for x in zip(seq_marker_data[:j1], marker_data[:j1])
                    ] + marker_data[j1:]
                if j2 < j1:
                    seq_marker_data = [
                        sum(x) for x in zip(seq_marker_data[:j2], marker_data[:j2])
                    ] + seq_marker_data[j2:]
                if j1 == j2:
    ...
    for x in seq_waveform_data:
        if N % 16 != 0:
            print(
                f"{16 - (N % 16)} points added to waveform data to ensure a multiple of 16 points are written to the AWG."
            )
            x += [0] * (16 - (N % 16))

    """Ensure number of samples are multiples of 16"""
    print(f"Number of samples in each channel modulo 16:{len(x) % 16}")
    ...
    if l_mark < l_seq:
        seq_marker_data += [0] * (l_seq - l_mark)
    elif l_seq < l_mark:
    ...
    marker_starts = [
        x[0]
        for x in enumerate(zip([0] + seq_marker_data[:-1], seq_marker_data))
    ...
    if len(marker_starts) > 2:
        print(
            "ERROR: There are more markers required than can be set currently using the marker channels!"
        )
        marker_starts = marker_starts[:2]

    print(f"Writing markers to marker channels at {marker_starts}")
    ...
    for channel, rel_offset, data in zip(awg_chs, channel_relative_offsets, seq_waveform_data):
        ...
        print(data)
        print(f"Rolling {channel} forward by {rel_offset} points")
        plt.plot(data)
        ...
        print(f"Writing {len(data)} points to {channel}")
        awg.set_active_channel(channel)
        if (
            channel == Channel.CHANNEL_1
            or channel == Channel.CHANNEL_2
            or channel == Channel.CHANNEL_3
        ):
