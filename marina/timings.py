...
def setup_awg(config):
    ...
    awg_config = AwgConfiguration(
        ...
        waveform_aom_calibrations_locations=list(
            config["AWG"]["waveform aom calibrations locations"]
        ),
    )

    tdc_config = TdcConfiguration(
        counter_channels=map(eval, config["TDC"]["counter channels"]),
...
def measure_signal(osc_manager, num_measurements=50, samp_rate=1e9, timebase_range=3e-6):
    ...
    for _ in range(num_measurements):
        data_stokes = osc_manager.acquire_with_trigger(
            4, samp_rate=samp_rate, timebase_range=timebase_range
        )
        data_pump = osc_manager.acquire_with_trigger(
            1, samp_rate=samp_rate, timebase_range=timebase_range
        )
...
stokes, std_stokes, pump, std_pump = measure_signal(
    osc_manager, num_measurements=50, samp_rate=1e9, timebase_range=3e-6
)
